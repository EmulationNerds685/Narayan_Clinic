...
const SlideItem = ({ slide, index, current }) => {
    ...
    >
      ...
      <div className="relative h-full flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl md:max-w-3xl lg:max-w-4xl w-full rounded-lg p-6 sm:p-8">
          ...
          <Link to={slide.link}>
            <button className="bg-[#3CAEA3] hover:bg-[#2b8c82] text-white px-5 py-2 sm:px-6 sm:py-2.5 rounded-full font-medium transition-colors duration-300 text-sm sm:text-base">
              {slide.buttonText}
            </button>
          </Link>
        </div>
    ...
    >
      ...
      >
        {slides.map((slide, index) => (
          <SlideItem key={index} slide={slide} index={index} current={current} />
        ))}
      </div>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 z-10">
        {slides.map((_, index) => (
          <button
            ...
            className={`h-2.5 w-2.5 sm:h-3 sm:w-3 rounded-full transition-all duration-300 ${index === current ? 'bg-[#3CAEA3] w-6 sm:w-8' : 'bg-gray-400 hover:bg-gray-300'
              }`}
